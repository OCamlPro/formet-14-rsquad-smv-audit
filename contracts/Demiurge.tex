...
\begin{itemize}
\item \issueCritical{Demiurge constructor}{No test is performed to
  verify the sender in the case {\tt msg.sender != address(0)}. An
  attacker could use it to deploy the contract himself for another
  user, providing its own {\tt addrStore}, i.e. with his own code for
  most contracts.}
...
\item \issueMajor{No initialization check performed}{The {\tt
    \_createChecks} function gives the false feeling the checks are
  performed for initialization of the Padawan and Proposal
  codes. However, the checks are not performed in the functions where
  they would be required. No attempt is done to perform the same
